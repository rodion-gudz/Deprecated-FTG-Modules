        ...
        if not message.is_private:
            ...
            if me.id == user.id:
                return await message.edit(
                    "<b>Ты не можешь себе давать предупреждение!</b>")
            ...
            if count == warns[chatid]["limit"]:
                ...
                try:
                    if warns[chatid]["action"] == "kick":
                        await message.client.kick_participant(int(chatid),
                                                              user.id)
                    elif warns[chatid]["action"] == "ban":
                        await message.client(
                            EditBannedRequest(int(chatid), user.id,
                    ...
                    return await message.edit(
                        "<b>У меня нет достаточных прав.</b>")
                else:
                    return await message.edit(
    ...
    async def getwarnscmd(self, message):
        ...
        if not message.is_private:
            ...
            try:
                ...
                return await message.edit(
                    "<b>Не удалось найти этого пользователя.</b>")
