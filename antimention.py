                ...
                if args:
                    if args == "kick":
                        ...
                        return await message.edit(
                            "<b>[AntiMention - Settings]</b> Такого режима нет в списке.\nДоступные режимы: "
                            "kick/ban/mute/none.")
                    self.db.set("AntiMention", "action", ac)
                    return await message.edit(
                        f"<b>[AntiMention - Settings]</b> Теперь при упоминании в чате будет выполняться действие: {ac[chatid]['action']}.")
                else:
                ...
                if str(userid) not in ac[chatid]["exceptions"]:
                    try:
                        if ac[chatid]["action"] == "kick":
                            ...
                            await message.client(EditBannedRequest(int(chatid), userid,
                                                                   ChatBannedRights(until_date=None,
                                                                                    view_messages=True)))
                        elif ac[chatid]["action"] == "mute":
                            await message.client(EditBannedRequest(int(chatid), userid,
                                                                   ChatBannedRights(until_date=True,
                                                                                    send_messages=True)))
                        else:
                            pass
                    except:
