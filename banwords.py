...
class BanWordsMod(loader.Module):
    ...
    async def addbwcmd(self, message):
        ...
        if not args: return await message.edit(
            "<b>[BanWords]</b> Нет аргументов.")
    ...
    async def rmbwcmd(self, message):
        """Удалить слово из список "Плохих слов". Используй: .rmbw <слово или
        all/clearall (по желанию)>.\nall - удаляет все слова из
    ...
    async def bwscmd(self, message):
        ...
        try:
            ...
            return await message.edit(
                "<b>[BanWords]</b> В этом чате нет списка слов.")
        word = ""
        for _ in ls:
            word += f"• <code>{_}</code>\n"
        await message.edit(
            f"<b>[BanWords]</b> Список слов в этом чате:\n\n{word}")

    async def bwstatscmd(self, message):
        """Статистика "Плохих слов". Используй: .bwstats <clear* (по
        ...
        if args == "clear":
            try:
                ...
                self.db.set("BanWords", "bws", words)
                return await message.edit(
                    "<b>[BanWords]</b> Настройки чата сброшены.")
            except KeyError:
                return await message.edit(
        ...
        try:
            ...
            for _ in words["stats"][chatid]:
                if _ not in ["action", "antimat", "limit"] and \
                        words["stats"][chatid][_] != 0:
            ...
            return await message.edit(
                "<b>[BanWords]</b> В этом чате нет тех, кто использовал "
                "спец.слова.")

    async def swbwcmd(self, message):
        """Переключить режим "Плохих слов". Используй: .swbw <режим(
        antimat/kick/ban/mute/none)>, или .swbw limit <кол-во:int>. """
        if not message.is_private:
            chat = await message.get_chat()
            if not chat.admin_rights and not chat.creator:
                return await message.edit("<b>Я не админ здесь.</b>")
        ...
        if args:
            if "limit" in args:
                try:
                    ...
                    self.db.set("BanWords", "bws", words)
                    return await message.edit(
                        f"<b>[BanWords]</b> Лимит спец.слов был установлен на {words['stats'][chatid]['limit']}.")
                except (IndexError, ValueError):
                    return await message.edit(
                        f"<b>[BanWords]</b> Лимит спец.слов в этом чате - {words['stats'][chatid]['limit']}\nУстановить новый можно командой .bwsw limit <кол-во:int>.")
            if args == "antimat":
                if not words["stats"][chatid]["antimat"]:
                    words["stats"][chatid]["antimat"] = True
                    self.db.set("BanWords", "bws", words)
                    return await message.edit(
                        "<b>[BanWords]</b> Режим \"антимат\" включен.")
                else:
                    words["stats"][chatid]["antimat"] = False
                    self.db.set("BanWords", "bws", words)
                    return await message.edit(
                        "<b>[BanWords]</b> Режим \"антимат\" выключен.")
            else:
                if args == "kick":
                    ...
                    return await message.edit(
                        f"<b>[BanWords]</b> Такого режима нет в списке. Есть: "
                        f"kick/ban/mute/none.")
                self.db.set("BanWords", "bws", words)
                return await message.edit(
                    f"<b>[BanWords]</b> Теперь при достижении лимита "
                    f"спец.слов будет выполняться действие: "
                    f"{words['stats'][chatid]['action']}.")
        else:
            return await message.edit(f"<b>[BanWords]</b> Настройки чата:\n\n"
                                      f"<b>Лимит спец.слов:</b> {words['stats'][chatid]['limit']}\n"
                                      f"<b>При достижении лимита спец.слов "
                                      f"будет выполняться действие:</b> "
                                      f"{words['stats'][chatid]['action']}\n "
                    ...
                    if count == words["stats"][chatid]["limit"]:
                        try:
                            if action == "kick":
                                ...
                                await message.client(eb(int(chatid), userid,
                                                        cb(until_date=None,
                                                           view_messages=True)))
                            elif action == "mute":
                                await message.client(eb(int(chatid), user.id,
                                                        cb(until_date=True,
                                                           send_messages=True)))
                            else:
                                pass
                            ...
                            await message.respond(
                                f"<b>[BanWords]</b> {user.first_name} достиг "
                                f"лимит ({words['stats'][chatid]['limit']}) "
                                f"спец.слова, и был ограничен в чате.")
                        except:
                            pass
                    await message.client.delete_messages(message.chat_id,
                                                         message.id)
        except:
            pass
